...
import (
	...
	"github.com/ppltools/webhookmanager/writer/atomic"
)
...
func prepareToWrite(dir string) error {
	...
	case os.IsNotExist(err):
		log.Infof(context.Background(), "cert directory doesn't exist, creating directory: %s", dir)
	...
	for _, f := range filenames {
		...
		if os.IsNotExist(err) {
			...
			log.Errorf(context.Background(), "unable to stat file %s: %s", abspath, err)
		}
		...
		if err != nil {
			...
			if err != nil {
				log.Errorf(context.Background(), "unable to remove old file %s: %s", abspath, err)
			}
		}
	}
	return nil
