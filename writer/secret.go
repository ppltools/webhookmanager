...
func (s *secretCertWriter) overwrite(resourceVersion string) (
	*generator.Artifacts, error) {
	...
	if err != nil {
		log.Infof(context.Background(), "Cert writer update secret failed: %v", err)
		return certs, err
	}
	log.Infof(context.Background(), "Cert writer update secret %s resourceVersion from %s to %s",
